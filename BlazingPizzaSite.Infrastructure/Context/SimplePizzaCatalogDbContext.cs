...
namespace BlazingPizzaSite.Infrastructure.Context
{
    public class SimplePizzaCatalogDbContext : DbContext
    {
        ...
        /*// exemple Fluent API validation
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<Pizza>().Property(p => p.Name).IsRequired().HasMaxLength(100);
            modelBuilder.Entity<Pizza>().Property(p => p.Category).IsRequired().HasMaxLength(100);//HasConversion<string>(); 
            
        }
        // exemple with data seeding
         protected override void OnModelCreating(ModelBuilder modelBuilder)
         {
             modelBuilder.Entity<Pizza>().HasData(
                 new Pizza { PizzaId = Guid.NewGuid(), Name = "Meat Pizza", Description = "Pepperoni, Sausage, Canadian Bacon, Bacon, Mozzarella", Category = Category.Meat },
                 new Pizza { PizzaId = Guid.NewGuid(), Name = "Chicken Pizza", Description = "Grilled Chicken, BBQ Sauce, Red Onion, Cilantro, Mozzarella", Category = Category.Chicken },
                 new Pizza { PizzaId = Guid.NewGuid(), Name = "Vegetarian Pizza", Description = "Mushrooms, Olives, Green Peppers, Onions, Mozzarella", Category = Category.Vegetarian },
                 new Pizza { PizzaId = Guid.NewGuid(), Name = "Vegan Pizza", Description = "Mushrooms, Olives, Green Peppers, Onions, Vegan Cheese", Category = Category.Vegan,  },
                 new Pizza { PizzaId = Guid.NewGuid(), Name = "Gluten-Free Pizza", Description = "Gluten-Free Crust, Mushrooms, Olives, Green Peppers, Onions, Mozzarella", Category = Category.GlutenFree },
                 new Pizza { PizzaId = Guid.NewGuid(), Name = "Nut-Free Pizza", Description = "Mushrooms, Olives, Green Peppers, Onions, Mozzarella", Category = Category.NutFree }
             );
         }*/
        /*  protected override void OnModelCreating(ModelBuilder modelBuilder)
          {
